...
class TrainModel:
    def __init__(self, args, train_dataset, model, tokenizer, logger, contrastive_weight=0.1, cross_validate_id=-1):
        self.args = args
        self.train_dataset = train_dataset
        self.model = model
        self.tokenizer = tokenizer
        self.logger = logger
        self.cross_validate_id = cross_validate_id
        self.contrastive_weight = contrastive_weight
    def collate_fn(self, batch_dataset: list):
        return_tuple = [[], [], [], [], [], [], []]
    ...
    def train(self):
        ...
        logger = self.logger
        cross_validate_id = self.cross_validate_id
        args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)
        train_sampler = RandomSampler(train_dataset)
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, collate_fn=self.collate_fn,drop_last=True)
        if args.max_steps > 0:
    ...
    #             shift_labels 4*149 shift_logits 4*149*50621
                ...
                loss1 = loss_fct(shift_logits1.view(-1, shift_logits1.size(-1)), shift_labels.view(-1)).reshape(shift_labels.size(0),-1)
    ...
    #             print(loss_weight + args.aug_data_weight*(1-loss_weight))
                ...
                weighted_loss1 = loss1*loss_weight + args.aug_data_weight*(1-loss_weight)*loss1
                loss = weighted_loss.sum()/(shift_labels.size(0)*shift_labels.size(-1)-k)
                loss1 = weighted_loss1.sum()/(shift_labels.size(0)*shift_labels.size(-1)-k)
                loss = (loss+loss1)/2
                if args.add_contrastive_loss:
                    ...
                    cl_mask = torch.ones_like(outputs[3][-1])
                    for k in range(cl_mask.size(0)):
                        begin_pos = batch[4][0]
                        cl_mask[k,:begin_pos,:] = 0
                    cl_mask.to(args.device)
                    input_embed = outputs[3][-1]*cl_mask
                    input_embed_aug = outputs1[3][-1]*cl_mask
                    target_embed = model(target, labels=labels)[3][-1]*cl_mask
                    contrastive_loss_ex = (contrastive_fct(input_embed.mean(dim=1),target_embed.mean(dim=1))+\
                    contrastive_fct(input_embed_aug.mean(dim=1),target_embed.mean(dim=1)))/2
#                     contrastive_loss_in = contrastive_fct(input_embed.mean(dim=1),input_embed_aug.mean(dim=1))
#                     contrastive_loss = (contrastive_loss_ex+contrastive_loss_in)/2
                    loss+=contrastive_loss_ex*self.contrastive_weight
...
def main():
    ...
    if not args.cross_validate:
        ...
        for i in range(NUM_FOLD):
            ...
            tokenizer.add_special_tokens(special_tokens_dict)
            model = model_class.from_pretrained(args.model_name_or_path + suffix,output_hidden_states=True)
            model.resize_token_embeddings(len(tokenizer))  # resize
            model.to(args.device)
            if args.block_size <= 0:
                args.block_size = tokenizer.max_len_single_sentence
            ...
            train_model = TrainModel(args,train_dataset,model,tokenizer,logger,cross_validate_id=i)
